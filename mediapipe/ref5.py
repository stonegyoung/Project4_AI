...
def check():
    w = np.random.randint(100, 1000, size=1000)
    h = np.random.randint(100, 600, size = 1000)
...
while cap.isOpened():
    ...
    for w,h in whlist:
        cv2.circle(img, (w,h), 3, (0,0,255), -1)
    ...
    if result.multi_hand_landmarks:
        for hand_landmarks, hand_lr in zip(result.multi_hand_landmarks, result.multi_handedness) :
            ...
            mp_drawing.draw_landmarks(
                ...
                mp_drawing.DrawingSpec((255,0,0), -1, 10), # 랜드마크 색
                # mp_drawing.DrawingSpec(), # 연결 선 색
            )
            ...
            for xy in whlist:
                if (xy[0]-70 <= point_x <= xy[0]+70) and (xy[1]-70 <= point_y <= xy[1]+70):
