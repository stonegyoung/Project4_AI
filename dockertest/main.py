...
log_dir = "./logs"  # 컨테이너 내부 경로
os.makedirs(log_dir, exist_ok=True)  # 디렉터리 생성

# logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

logger1 = logging.getLogger('general_logger')
logger1.setLevel(logging.INFO)  # 최소 레벨을 INFO로 설정
handler1 = logging.FileHandler(os.path.join(log_dir, 'info.log'))  # history.log에 기록
...
handler2 = logging.FileHandler(os.path.join(log_dir, 'history.log'), encoding='utf-8')  # error.log에 기록
...
vectorstore = Chroma(embedding_function=OpenAIEmbeddings(), persist_directory='./OceanDB')
